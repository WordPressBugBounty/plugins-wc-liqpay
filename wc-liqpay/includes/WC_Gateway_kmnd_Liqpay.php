...
class WC_Gateway_kmnd_Liqpay extends WC_Payment_Gateway {
    ...
    public function process_payment($order_id) {
        ...
        $params = array(
            ...
            'result_url'    => add_query_arg(
                array('wc-api' => 'liqpay_return', 'order_id' => $order->get_id()),
                home_url('/')
            ),
    ...
    public function handle_callback() {
        ...
        if (!$data || !$signature) {
            ...
            wp_die('Invalid data received', 'LiqPay Callback', ['response' => 400]);
        }
        ...
        if ($signature !== $generated_signature) {
            wp_die('Signature verification failed', 'LiqPay Callback', ['response' => 400]);
        }
        ...
        if (!$order_id || !$status) {
            wp_die('Missing order ID or status', 'LiqPay Callback', ['response' => 400]);
        }
        ...
        $order = wc_get_order($order_id);
        if (!$order) {
            wp_die('Order not found', 'LiqPay Callback', ['response' => 404]);
        }
